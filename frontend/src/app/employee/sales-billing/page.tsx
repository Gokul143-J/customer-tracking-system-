...
import {
  ShoppingBag, Search, Loader2, IndianRupee, CheckCircle2, FileText, RefreshCw,
} from "lucide-react";
...
export default function SalesBillingPage() {
  ...
  async function loadSales() {
    setLoading(true);
    try { const s = await salesApi.list(); setSales(s); } finally { setLoading(false); }
  }
  ...
  async function searchTicket() {
    ...
    try {
      ...
      const t = all.find((tk: any) => tk.ticket_number?.toLowerCase() === query.trim().toLowerCase() || tk.id === query.trim());
  ...
  function computeTotal() {
    ...
    setForm((f) => ({ ...f, final_amount: Math.max(0, toNum(f.making_charges) + toNum(f.gst_amount) - toNum(f.discount)).toFixed(2) }));
  }
  ...
  async function submitSale() {
    ...
    try {
      ...
      await salesApi.create({
        ...
        total_weight: Number(form.total_weight || 0), making_charges: Number(form.making_charges || 0),
        stone_weight: Number(form.stone_weight || 0), gst_amount: Number(form.gst_amount || 0),
        discount: Number(form.discount || 0), final_amount: Number(form.final_amount || 0),
        invoice_number: invoiceNum, payment_method: form.payment_method, status: "completed",
      });
      await ticketsApi.close(ticket.id, "COMPLETED");
      setLastInvoice(invoiceNum); setTicket(null); setQuery("");
        ...
        <div className="xl:col-span-2 rounded-2xl bg-white border border-gray-100 p-6 shadow-sm">
          ...
          <div className="flex gap-3 mb-4">
            <div className="flex-1 relative">
              ...
              <input className="w-full rounded-xl border border-gray-200 bg-white pl-10 pr-4 py-3 text-sm placeholder:text-gray-400 focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-100 transition" placeholder="Enter ticket number…" value={query} onChange={(e) => setQuery(e.target.value)} onKeyDown={(e) => e.key === "Enter" && searchTicket()} />
            </div>
          ...
          {ticket && (
            <div className="p-4 rounded-xl bg-gradient-to-r from-indigo-50 to-white border border-indigo-200 mb-5">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-indigo-400 to-indigo-600 text-white flex items-center justify-center"><FileText className="w-5 h-5" /></div>
                <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div><label className="block text-xs font-semibold uppercase tracking-wider text-gray-500 mb-2">Total Weight (g)</label>
              <input type="number" step="0.001" className="w-full rounded-xl border border-gray-200 bg-white px-4 py-3 text-sm focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-100 transition" value={form.total_weight} onChange={(e) => setForm({ ...form, total_weight: e.target.value })} /></div>
            <div><label className="block text-xs font-semibold uppercase tracking-wider text-gray-500 mb-2">Stone Weight (carats)</label>
              <input type="number" step="0.001" className="w-full rounded-xl border border-gray-200 bg-white px-4 py-3 text-sm focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-100 transition" value={form.stone_weight} onChange={(e) => setForm({ ...form, stone_weight: e.target.value })} /></div>
            <div><label className="block text-xs font-semibold uppercase tracking-wider text-gray-500 mb-2">Making Charges (₹)</label>
              <input type="number" className="w-full rounded-xl border border-gray-200 bg-white px-4 py-3 text-sm focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-100 transition" value={form.making_charges} onChange={(e) => setForm({ ...form, making_charges: e.target.value })} onBlur={computeTotal} /></div>
            <div><label className="block text-xs font-semibold uppercase tracking-wider text-gray-500 mb-2">GST Amount (₹)</label>
              <input type="number" className="w-full rounded-xl border border-gray-200 bg-white px-4 py-3 text-sm focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-100 transition" value={form.gst_amount} onChange={(e) => setForm({ ...form, gst_amount: e.target.value })} onBlur={computeTotal} /></div>
            <div><label className="block text-xs font-semibold uppercase tracking-wider text-gray-500 mb-2">Discount (₹)</label>
              <input type="number" className="w-full rounded-xl border border-gray-200 bg-white px-4 py-3 text-sm focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-100 transition" value={form.discount} onChange={(e) => setForm({ ...form, discount: e.target.value })} onBlur={computeTotal} /></div>
            <div><label className="block text-xs font-semibold uppercase tracking-wider text-gray-500 mb-2">Payment Method</label>
            ...
            <div className="col-span-2"><label className="block text-xs font-semibold uppercase tracking-wider text-gray-500 mb-2">Final Amount ()</label>
              <div className="relative"><IndianRupee className="w-4 h-4 absolute left-3.5 top-3.5 text-gray-400" />
                <input type="number" className="w-full rounded-xl border border-gray-200 bg-white pl-10 pr-4 py-3 text-lg font-bold focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-100 transition" value={form.final_amount} onChange={(e) => setForm({ ...form, final_amount: e.target.value })} /></div></div>
          </div>
